...
int askQuestions3(std::istream &is, std::ostream &os)
{
    ...
    {
        os << i << space << stringvalue << space << doublevalue;
        os << std::endl;
    }
    // os <<std::endl;
    return intvalue;
...
int askHeroQuestions(std::istream& is, std::ostream& os){
    std::string stringprompt = "Who is your hero? ";
    std::string stringvalue = getString(is, os, stringprompt);
    std::string intprompt = "What year were they born? ";
    double intvalue = getDouble(is, os, intprompt);
    os<<stringprompt<<"was born in "<<intvalue<< ".";
    return intvalue;
